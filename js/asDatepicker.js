...
(function($, document, window, undefined) {
  ...
  var Plugin = ($[pluginName] = function(element, options) {
    ...
    this.$inputWrap = this.$el
      .addClass(this.namespace + '-input')
      .wrap(
        this.options.tplInputWrapper().replace(/namespace/g, this.namespace)
      )
      .parent();
  ...
  Plugin.prototype = {
    ...
    _init: function() {
      ...
      this.outputFormat = this._parseFormat(
      ...
      if (this.isMobile) {
        ...
        this.$picker.append(title).addClass(this.namespace + '_isMobile');
    ...
    _initSections: function() {
      ...
      this.calendarCaptions = this.calendars.find(
        '.' + this.namespace + '-caption'
      );
      ...
      this.buttonCancels = this.calendars.find(
        '.' + this.namespace + '-button-cancel'
      );
      this.buttonSaves = this.calendars.find(
        '.' + this.namespace + '-button-save'
      );
    },
    ...
    _initDate: function() {
      var date =
        this.options.date === 'today'
          ? new Date()
          : this._parseDate(this.options.date, this.format);
      this._date = {};
      ...
      for (var i = 0; i < this.calendarsNum; i++) {
        ...
        if (this.mode === 'multiple') {
          this._setDate(
            this._date.currentDate[i],
            'month',
            this._date.currentDate[i].getMonth() + i
          );
        } else {
          this._date.selectedDate[i] =
            this._date.selectedDate[i] || new Date(date);
    ...
    _manageViews: function(index) {
      switch (this.views[index]) {
        ...
        case 'months':
          ...
          this.calendars
            .eq(index)
            .removeClass(this.namespace + '_days')
    ...
    _generateDaypicker: function(index) {
      this._generateHeader(
        index,
        LABEL[this.options.lang].months[this._date.currentMonth[index]] +
          ' ' +
          this._date.currentYear[index]
      );
    ...
    _generateYearpicker: function(index) {
      this._generateHeader(
        ...
        this._date.currentYear[index] -
          ...
          ' ' +
          (this._date.currentYear[index] + 4)
      );
      ...
      for (var j = 0; j < 42; j++) {
        ...
        if (j < daysFromPrevMonth) {
          ...
          if (this.hasKeyboard) {
            if (this.mode === 'multiple') {
              if (
                Date.parse(dateArray[j]) === Date.parse(this._date.focusDate)
              ) {
              ...
              if (
                ...
                Date.parse(this._date.focusDate[index])
              ) {
                className += ' ' + this.namespace + '_focus';
              }
            }
        ...
        case 'range':
          if (index === 0) {
            switch (this.views[index]) {
              ...
              case 'years':
                current = new Date(
                  this._date.currentYearDate[index]
                ).setFullYear(this._date.currentYear[index] + 4);
                selected = Date.parse(this._date.selectedYearDate[1]);
                break;
            }
            ...
            switch (this.views[index]) {
              ...
              case 'years':
                current = new Date(
                  this._date.currentYearDate[index]
                ).setFullYear(this._date.currentYear[index] - 7);
                selected = Date.parse(this._date.selectedYearDate[0]);
                break;
            }
              ...
              if (!status) {
                switch (date.length) {
                  ...
                  case 2:
                    status = true;
                    if (
                      max < Date.parse(date[0]) ||
                      min > Date.parse(date[1])
                    ) {
                      status = false;
                    }
                    break;
    ...
    _isSelectable: function(view, y, m, d) {
      var isSelectable = true,
        ...
        selectableDate = this._parseDateArr(
        ...
        selectableDayOfWeek = this._parseDateSection(
      ...
      if (isSelectable && selectableDate.length > 0) {
        isSelectable = this._judgeSections(
          'date',
          _curr,
          selectableDate,
          _isDay
    ...
    _parseDateArr: function(arr, format) {
      ...
      for (var i = 0; i < arr.length; i++) {
        if (typeof arr[i] === 'string') {
          ...
          array[count++] = [
            this._parseDate(from, format),
            this._parseDate(to, format)
          ];
        }
      }
      return array;
    ...
    _updateDate: function(i) {
      ...
      this._date.currentMonthDate[i] = new Date(
        this._date.currentYear[i],
        this._date.currentMonth[i],
        1,
        0,
        0,
        0,
      ...
      this._date.currentYearDate[i] = new Date(
        this._date.currentYear[i],
        0,
        1,
        0,
      ...
      if (this.mode !== 'multiple') {
        ...
        this._date.selectedMonthDate[i] = new Date(
          this._date.selectedYear[i],
          this._date.selectedMonth[i],
          1,
          0,
          0,
          0,
        ...
        this._date.selectedYearDate[i] = new Date(
          this._date.selectedYear[i],
          0,
          1,
          0,
    ...
    _position: function() {
      var calendar_height = this.$picker.outerHeight(),
        ...
        input_left = this.$el.offset().left - 40,
    ...
    _trigger: function(eventType) {
      var data =
        arguments.length > 1
          ? Array.prototype.slice.call(arguments, 1).push(this)
          : this;
      // event
    ...
    _click: function(e) {
      ...
      if (
        $target.closest(this.$inputIcon).length === 0 &&
        $target.closest(this.$picker).length === 0 &&
        ...
        $target.closest(this.$el).length !== 1 &&
        $target.closest(this.$picker).length === 1
      ) {
        ...
        if (_targetSpan.length === 1) {
          var j = _targetSpan
            .parents('.' + this.namespace + '-content')
            .index();

          if (
            !_targetSpan.hasClass(this.namespace + '_otherMonth') &&
            !_targetSpan.hasClass(this.namespace + '_untouchable') &&
            _targetSpan.parent('.' + this.namespace + '-head').length !== 1
          ) {
        ...
        if (_target.parent('.' + this.namespace + '-buttons').length === 1) {
          var k = _target.parents('.' + this.namespace + '-content').index(),
            flag =
              _target[0].className === this.namespace + '-button-save'
                ? true
                : false;
          ...
          switch (this.options.mode) {
            ...
            case 'multiple':
              ...
              if ($.inArray(date, this._date.selectedDate) > -1) {
                ...
                if (
                  this._date.selectedDate.length < this.options.multipleSize
                ) {
    ...
    _setValue: function() {
      switch (this.mode) {
        case 'single':
          var formated = this._formatDate(
            this._date.selectedDate[0],
            this.outputFormat
          );
          this.$el.val(formated);
          break;
        case 'range':
          var formatedStart = this._formatDate(
              this._date.selectedDate[0],
              this.outputFormat
            ),
            formatedEnd = this._formatDate(
              this._date.selectedDate[1],
              this.outputFormat
            );
          this.$el.val(
            formatedStart +
        ...
        case 'multiple':
          ...
          for (var j = 0; j < this._date.selectedDate.length; j++) {
            _formated = this._formatDate(
              this._date.selectedDate[j],
              this.outputFormat
            );
            if (val.length === 0) {
              val += _formated;
      ...
      switch (this.views[i]) {
        case 'days':
          ...
          if (this.mode === 'multiple') {
            if (isTurning) {
              if (this.focused === 0) {
                for (var j = 0; j < this.calendarsNum; j++) {
                  this._date.currentDate[j].setMonth(
                    this._date.currentMonth[j] - 1
                  );
                  this._updateDate(j);
                  this._manageViews(j);
                }
              ...
              this._date.focusDate.setMonth(
                this._date.focusDate.getMonth() - 1
              );
              for (var k = 0; k < this.calendarsNum; k++) {
                this._date.currentDate[k].setMonth(
                  this._date.currentMonth[k] - 1
                );
                this._updateDate(k);
                this._manageViews(k);
              }
            ...
            if (this.hasKeyboard) {
              ...
              this._date.focusDate[i] = new Date(
                date.getFullYear(),
                date.getMonth(),
                this._date.focusDate[i].getDate(),
                0,
                0,
                0,
                0
        ...
        case 'months':
          ...
          if (this.hasKeyboard) {
            this._date.focusDate[i] = new Date(
              date.getFullYear(),
              this._date.focusDate[i].getMonth(),
              this._date.focusDate[i].getDate(),
              0,
              0,
              0,
        ...
        case 'years':
          ...
          if (this.hasKeyboard && isTurning) {
            this._date.focusDate[i] = new Date(
              this._date.focusDate[i].getFullYear() - 12,
              this._date.focusDate[i].getMonth(),
              this._date.focusDate[i].getDate(),
              0,
              0,
    ...
    mobileInteDate: function(index) {
      ...
      if (this.mode === 'multiple') {
        ...
        this._date.currentDate[index] = new Date(
          this._date.selectedDate[index]
        );
      }
    ...
    dateTransform: function(fromDate, toDate) {
      ...
      $.each(fromDate.selectedDate, function(n, v) {
        ...
        toDate.selectedDate[n] =
          self.mode === 'multiple' ? Date.parse(date) : date;
      });
    },
    show: function() {
      var self = this;
    ...
    _keyboard: {
      ...
      prevDate: function() {
        var i = this.focused,
          date =
            this.mode === 'multiple'
              ? this._date.focusDate
              : this._date.focusDate[i],
          hasLocked =
            this.mode === 'multiple'
              ? false
              : this.calendarPrevs
                  .eq(this.focused)
                  .hasClass(this.namespace + '_blocked');
        ...
        switch (this.views[i]) {
          case 'days':
            if (
              ...
              new Date(this._date.currentDate[i]).setMonth(
                this._date.currentMonth[i] - 1
              )
            ) {
              if (!hasLocked) {
      ...
      updateValue: function(self) {
        var i = self.focused,
          date =
            self.mode === 'multiple'
              ? self._date.focusDate
              : self._date.focusDate[i];
        if (
          !self.calendars
            .eq(i)
            .find('.' + self.namespace + '_focus')
            .hasClass(self.namespace + '_untouchable')
        ) {
          switch (self.views[i]) {
            case 'days':
          ...
          switch (this.mode) {
            case 'single':
              ...
              if (_date) {
                ...
                this._date.currentDate[0] = new Date(
                  this._date.selectedDate[0]
                );
                this._updateDate(0);
                this._manageViews(0);
            ...
            case 'range':
              ...
              if (judge === true) {
                ...
                for (var i = 0; i < 2; i++) {
                  this._date.currentDate[i] = new Date(
                    this._date.selectedDate[i]
                  );
                  this._updateDate(i);
                  this._manageViews(i);
